...
//        promotable.forEach(System.out::println);
//
...
//                // X loses dominance at each y in Y
//                for (var y : Y) {
//                    // for each block y where X loses dominance, we need to place a phi in y for variable v
//                    if (!phiPresent(placed, y, v)) {
//                        var temp = nextTemp();
//                        var phi = new Phi(temp, null); // TODO null
/// /                        phi.addIncoming(X.getLabel(), null); // TODO null should not be here
//                        // place placeholder, empty phi in the block
//                        placed.computeIfAbsent(y, k -> new HashMap<>()).put(v, phi);
//                    } else {
//                        // phi already placed, but we need to add this block's label
////                        var phi = placed.get(y).get(v);
////                        phi.addIncoming(X.getLabel(), null); // TODO ditto
//                    }
...
//
//        /**
//         * Builds a map from basic blocks to the set of blocks that they success.
//         * 
//         * @param blocks the basic blocks to build the successors map for
//         * @return a map from basic blocks to the set of blocks that they success
//         */
...
//                var inst = it.next();
//
//                if (inst instanceof Load(var dst, var src) && promotables.contains(src)) {
//                    IRTemp cur = topOrUndef(block, src);
//                    it.set(new Mov(dst, cur)); // replace Load with Mov of the temp (either defined or undef)
//                    continue;
//                }
//
//                if (inst instanceof Store(var dst, var src) && promotables.contains(dst)) {
//                    it.remove(); // kill the store
//                    IRTemp name = ensureTempAtCursor(block, it, src); // materialize value to a temp
//                    stack.get(dst).push(name); // new SSA def for slot
//                    continue;
//                }
//
